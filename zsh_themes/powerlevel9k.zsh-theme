...
prompt_rbenv() {
  if [[ -n "$RBENV_VERSION" ]]; then
    "$1_prompt_segment" "$0" "$2" "red" "white" "$RBENV_VERSION" 'RUBY_ICON'
  elif [ $commands[rbenv] ]; then
    local rbenv_version_name="$(rbenv version-name)"
    local rbenv_global="$(rbenv global)"
    if [[ "${rbenv_version_name}" != "${rbenv_global}" || "${POWERLEVEL9K_RBENV_PROMPT_ALWAYS_SHOW}" == "true" ]]; then
      "$1_prompt_segment" "$0" "$2" "red" "white" "$rbenv_version_name" 'RUBY_ICON'
    fi
  fi
}
...
prompt_pyenv() {
  if [[ -n "$PYENV_VERSION" ]]; then
    "$1_prompt_segment" "$0" "$2" "blue" "yellow" "$PYENV_VERSION" 'PYTHON_ICON'
    ...
    if [[ "${pyenv_version_name}" != "${pyenv_global}" || "${POWERLEVEL9K_PYENV_PROMPT_ALWAYS_SHOW}" == "true" ]]; then
      "$1_prompt_segment" "$0" "$2" "blue" "yellow1" "$pyenv_version_name" 'PYTHON_ICON'
    fi
  fi
}
...
prompt_java_version() {
  ...
  java_version=$(java -version 2>/dev/null && java -fullversion 2>&1 | cut -d '"' -f 2 | cut -d + -f 1)

  if [[ -n "$java_version" ]]; then
    "$1_prompt_segment" "$0" "$2" "red" "darkblue" "$java_version" "JAVA_ICON"
  fi
}
